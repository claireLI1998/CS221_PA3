...
for(int i = 0; i < im.width(); i++){
	for(int j =0; j < im.height(); j++) {
		if(i > 0 && j > 0) {
			sumHueX[i][j] = cos(((im.getPixel(i,j) -> h) * PI) / 180)
...
for(int i = 0; i < im.width(); i++){
	for(int j =0; j < im.height(); j++) {
		if(i > 0 && j > 0) {
			sumHueY[i][j] = sin((PI * (im.getPixel(i,j) -> h)) / 180)
...
/* YOUR CODE HERE */
    ...
    if(ulx == 0 && uly == 0){
        ...
        double hue = atan2(sumHueY[lrx][lry], sumHueX[lrx][lry])*180/PI;
        avgHue = (hue>=0)? hue: (hue+360);
    }else if(ulx == 0){
        avgSat = (sumSat[lrx][lry] - sumSat[lrx][uly - 1])/sum;
        avgLum = (sumLum[lrx][lry] - sumLum[lrx][uly - 1])/sum;
        double hue = atan2(sumHueY[lrx][lry] - sumHueY[lrx][uly - 1], sumHueX[lrx][lry] - sumHueX[lrx][uly - 1])*180/PI;
        avgHue = (hue>=0)? hue: (hue+360);
    }else if(uly == 0){
    	avgSat = (sumSat[lrx][lry] - sumSat[ulx - 1][lry])/sum;
        avgLum = (sumLum[lrx][lry] - sumLum[ulx - 1][lry])/sum;
        double hue = atan2(sumHueY[lrx][lry] - sumHueY[ulx - 1][lry], sumHueX[lrx][lry] - sumHueX[ulx - 1][lry])*180/PI;
        avgHue = (hue>=0)? hue: (hue+360);
    }else{
    	...
    	double hue = atan2(sumHueY[lrx][lry] - sumHueY[lrx][uly - 1] - sumHueY[ulx - 1][lry] + sumHueY[ulx - 1][uly - 1],
